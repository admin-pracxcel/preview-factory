...
- `services[]`: **6–12** service-detail pages — `slug`, `title`, `summary`, `icon`, `starting_price?`, `intro` (2+ paragraphs), `benefits[]` (4–6), `sections[]` (1–3 heading+body blocks), `faqs[]` (0–3, each with an `id` field formatted `faq-<service-slug>-<n>`), `seo`
- `locations[]`: **8–20** suburb pages — `slug`, `suburb`, `state`, `intro`, `body`, `landmarks[]`, `services_offered[]` (service slugs or titles), `sections[]` (1–2 heading+body blocks), `benefits[]`, `faqs[]` (each with `id` formatted `faq-<suburb-slug>-<n>`), `seo`
- `service_areas[]`: **5–15** service-in-area landing pages — `slug` = `${service_slug}-${suburb-slug}`, `service_slug`, `service_title`, `suburb`, `headline`, `body`, `benefits[]`, `sections[]`, `faqs[]` (each with `id` formatted `faq-<area-slug>-<n>`), `seo`
- `faq`: site-wide FAQ (`items[]` each with `id` formatted `faq-general-<n>`, `seo`)
- `about`: rich about page (`body`, `photo_url?`, `years_in_business?`, `licence?` for AHPRA registration, `values[]`, `seo`)
