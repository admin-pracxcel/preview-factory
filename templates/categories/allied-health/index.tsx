/**
 * Allied-health category — multi-page site renderer.
 *
 * Given a validated `SiteProps` object, a `basePath` (where the site is mounted,
 * e.g. "/preview/allied-health") and a `slug[]` from the catch-all route, this
 * resolves and renders the correct page. Also exposes per-page metadata for
 * Next.js `generateMetadata`.
 *
 * Covers the lead niche (physiotherapy) and other allied-health niches
 * (occupational therapy, speech pathology, dietetics, etc.) — niche differences
 * are handled at generation time via the category system prompt, not by
 * separate components.
 *
 * Visual design: teal/slate palette, trust-forward layout, prominent booking CTA,
 * AHPRA-compliant copy framing throughout.
 */
...
export function renderAlliedHealthPage(
  site: SiteProps,
  slug: string[],
  basePath: string,
  tenantId?: string,
  ...
  // Home
  if (!slug || slug.length === 0) return <HomePage site={site} basePath={basePath} tenantId={tenantId} showBlog={showBlog} />;
...
export function alliedHealthPageMetadata(
  site: SiteProps,
  slug: string[],
