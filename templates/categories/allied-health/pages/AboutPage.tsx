/**
 * Allied-health category — about page.
 *
 * Prominently displays AHPRA registration, credentials, and years in practice.
 * Uses LocalBusiness + MedicalBusiness JSON-LD for E-E-A-T signals.
 */
...
export function AboutPage({ site, basePath }: { site: SiteProps; basePath: string }) {
  ...
  // LocalBusiness + MedicalBusiness JSON-LD for E-E-A-T
  const bizLd = buildLocalBusinessJsonLd(site);
  const jsonLd = [
    { ...bizLd, "@type": ["LocalBusiness", "MedicalBusiness"] },
        ...
        <div className="mx-auto grid max-w-6xl items-start gap-10 px-4 lg:grid-cols-2">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {about.licence && (
                <span className="inline-flex items-center gap-2 rounded-full bg-teal-50 px-4 py-2 text-sm font-semibold text-zinc-700">
          ...
          {about.photo_url && (
            ...
            >
              ...
              {typeof about.years_in_business === "number" && (
                <div className="absolute bottom-4 left-4 rounded-xl bg-[var(--primary)] px-4 py-3 text-[var(--primary-fg)] shadow-lg">
