/**
 * Allied-health category — location (suburb) page.
 *
 * Highlights the practice's presence and services in a specific suburb.
 * Uses LocalBusiness JSON-LD scoped to the suburb location.
 */
...
export function LocationPage({
  ...
  // Services to surface for this suburb
  ...
  // Service-in-area pages for this suburb
    ...
    <SiteShell site={site} basePath={basePath} jsonLd={jsonLd}>
      ...
      <section className="py-14 sm:py-20">
        <div className="mx-auto max-w-4xl px-4 space-y-10">
          ...
          {page.landmarks.length > 0 && (
            <div>
              <p className="mb-3 text-sm font-semibold uppercase tracking-wide text-zinc-400">
                Areas we serve in {page.suburb}
              </p>
              <div className="flex flex-wrap gap-2.5">
                {page.landmarks.map((lm) => (
                  <span
                    key={lm}
                    className="inline-flex items-center gap-1.5 rounded-full bg-teal-50 px-4 py-2 text-sm font-medium text-zinc-700"
                  >
          ...
          {page.benefits.length > 0 && (
            <BenefitsList items={page.benefits} heading={`Why ${page.suburb} patients choose us`} />
          )}
        </div>
      </section>
      ...
      {serviceCards.length > 0 && (
        <ServicesGrid
          ...
          basePath={basePath}
          heading={`Physiotherapy services in ${page.suburb}`}
          subheading={`Available to patients from ${page.suburb} and nearby suburbs. Registered physiotherapists, evidence-based treatment.`}
        />
      )}
      ...
      {nearby.length > 0 && <RelatedLinks heading="Nearby areas we serve" links={nearby} />}

      <CtaBand
        heading={`Located in ${page.suburb}?`}
        body="Book an appointment with our registered physiotherapists — same-day appointments often available."
        cta={cta}
      />
    </SiteShell>
  );
