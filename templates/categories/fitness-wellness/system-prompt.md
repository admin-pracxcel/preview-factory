...
- `services[]`: **6–10** service-detail pages — `slug`, `title`, `summary`, `icon`, `starting_price?`, `intro` (2+ paragraphs), `benefits[]` (4–6), `sections[]` (1–3 heading+body blocks), `faqs[]` (0–3 items, each with `id` in format `faq-<service-slug>-<n>`), `seo`
- `locations[]`: **8–16** suburb pages — `slug`, `suburb`, `state`, `intro`, `body`, `landmarks[]`, `services_offered[]` (service slugs), `seo`
- `service_areas[]`: **5–12** service-in-area landing pages — `slug` = `${service_slug}-${suburb-slug}`, `service_slug`, `service_title`, `suburb`, `headline`, `body`, `benefits[]`, `seo`
- `faq`: site-wide FAQ (`items[]` — each item needs an `id` in format `faq-site-<n>`, `seo`)
- `about`: rich about page (`body`, `photo_url?`, `years_in_business?`, `licence?`, `values[]`, `seo`)

Total target: **20–35 indexable pages**. Every page needs a unique, keyword-aware `seo.title` and `seo.description`. Interlink naturally (services ↔ areas ↔ locations).
