/**
 * Fitness-wellness category — multi-page site renderer.
 *
 * Given a validated `SiteProps` object, a `basePath` (where the site is mounted,
 * e.g. "/preview/fitness-wellness") and a `slug[]` from the catch-all route, this
 * resolves and renders the correct page. Also exposes per-page `<title>/<description>`
 * metadata for Next.js `generateMetadata`.
 *
 * Covers fitness and wellness niches: personal training, group fitness, online coaching,
 * rehabilitation training, strength and conditioning, etc. Niche differences are handled
 * at generation time via the category system prompt, not by separate components.
 */
...
export function renderFitnessPage(
  site: SiteProps,
  slug: string[],
  basePath: string,
  tenantId?: string,
...
export function fitnessPageMetadata(
  site: SiteProps,
  slug: string[],
