/**
 * Beauty & Aesthetics category — multi-page site renderer.
 *
 * Given a validated `SiteProps` object, a `basePath` (where the site is mounted,
 * e.g. "/preview/beauty-aesthetics") and a `slug[]` from the catch-all route,
 * this resolves and renders the correct page. Also exposes per-page
 * `<title>/<description>` metadata for Next.js `generateMetadata`.
 *
 * Lead niche: hair salon / hairdresser. The same components handle beauty
 * clinics, nail bars, lash studios, brow bars and related niches — niche
 * differences are handled at generation time via the category system prompt.
 */
...
export function renderBeautyPage(
  site: SiteProps,
  slug: string[],
  basePath: string,
  tenantId?: string,
...
export function beautyPageMetadata(
  site: SiteProps,
  slug: string[],
