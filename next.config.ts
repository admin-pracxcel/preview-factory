...
const nextConfig: NextConfig = {
  ...
  async headers() {
    const csp = [
      ...
      //
      // 'wasm-unsafe-eval' — Chrome requires this even when 'unsafe-eval'
      // is set, because WASM is treated as a separate directive since
      // Chrome 84+. Needed by @imgly/background-removal on logo upload.
      //
      // blob: — @imgly creates blob URLs for its runtime worker script.
      // Same-origin blob URL, generated by our own client bundle.
      // Analytics loaders (marketing host only, gated in ClarityAnalytics /
      // MetaPixel / GoogleTagManager components — CSP still needs the hosts
      // allowlisted because CSP is applied to every response regardless):
      //   googletagmanager.com  GTM loader
      ...
      // Analytics beacons: GA4 (analytics.google.com + region1), GTM
      // (googletagmanager.com), Clarity (*.clarity.ms), Meta (facebook.com).
      "connect-src 'self' blob: https://*.supabase.co https://*.ingest.us.sentry.io https://*.ingest.sentry.io https://cloudflareinsights.com https://staticimgly.com https://www.googletagmanager.com https://www.google-analytics.com https://analytics.google.com https://region1.google-analytics.com https://*.clarity.ms https://www.facebook.com",
